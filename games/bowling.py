...
class BowlingGame(BaseGame):
    ...
    async def play(self, message):
        try:
            ...
            bowling_msg = await message.answer_dice(emoji="🎳")
            await asyncio.sleep(4)
            score = bowling_msg.dice.value
            ...
            logger.error(f"Ошибка в игре боулинг для пользователя {message.from_user.id}: {str(e)}")
